...
const redelegationStatusError = (a, t) => {
  if (a.solanaResources) {
    const redelegations = a.solanaResources.redelegations;
    ...
    if (
      redelegations.some((redelegation) => {
        ...
        return (
          dstValidator === t.solanaSourceValidator &&
          redelegation.completionDate > new Date()
        );
      })
    )
      return new SolanaRedelegationInProgress();

    if (t.solanaSourceValidator === t.validators[0].address)
      return new InvalidAddressBecauseDestinationIsAlsoSource();
  }

  return isDelegable(a, t.solanaSourceValidator, t.validators[0].amount);
};
...
const getSendTransactionStatus = async (a, t, isPreValidation = false) => {
  ...
  if (
    a.solanaResources &&
    a.solanaResources.delegations.length > 0 &&
    t.useAllAmount
  ) {
...
const getDelegateTransactionStatus = async (a, t, isPreValidation = false) => {
  ...
  if (t.validators.length > COSMOS_MAX_DELEGATIONS) {
    errors.validators = new SolanaTooManyValidators();
  }
  ...
  if (totalSpent.eq(a.spendableBalance)) {
    warnings.delegate = new SolanaDelegateAllFundsWarning();
  }
...
const getTransactionStatus = async (a, t, isPreValidation = false) => {
  ...
  if (t.mode === "redelegate") {
    ...
    invariant(
      a.solanaResources &&
        a.solanaResources.unbondings.length < COSMOS_MAX_UNBONDINGS,
  ...
  if (["claimReward", "claimRewardCompound"].includes(t.mode)) {
    ...
    const claimReward = t.validators.length
      ? solanaResources.delegations.find(
          (delegation) =>
