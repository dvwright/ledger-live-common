...
import type { SolanaValidatorItem, SolanaRewardsState } from "./types";
import type { CryptoCurrency } from "../../types";

const getBaseApiUrl = (currency: CryptoCurrency) => {
  if (currency.id === "solana_testnet") {
...
const cacheValidators = makeLRUCache(
  async (rewardState: SolanaRewardsState, currency: CryptoCurrency) => {
  ...
  (_: SolanaRewardsState, currency: CryptoCurrency) => currency.id
);
...
const getRewardsState = makeLRUCache(
  async (currency: CryptoCurrency) => {
    ...
    // Source for seconds per year : https://github.com/gavinly/SolanaParametersWiki/blob/master/Mint.md#notes-3
...
const getStargateRewardsState = makeLRUCache(
  async (_currency: CryptoCurrency) => {
    ...
    const communityTaxUrl = `${getBaseApiUrl(
      currency
    )}/solana/distribution/v1beta1/params`;
    const { data: communityTax } = await network({
      url: communityTaxUrl,
      method: "GET",
    });
    ...
    const supplyUrl = `${getBaseApiUrl(currency)}/solana/bank/v1beta1/supply/${
    ...
    const ratioUrl = `${getBaseApiUrl(currency)}/solana/staking/v1beta1/pool`;
    const { data: ratioData } = await network({ url: ratioUrl, method: "GET" });
    const actualBondedRatio =
...
const computeAvgYearlyInflation = (rewardsState: SolanaRewardsState) => {
...
export const validatorEstimatedRate = (
  ...
  rewardsState: SolanaRewardsState
) => {
...
export const hydrateValidators = (validators: SolanaValidatorItem[]) => {
  log("solana/validators", "hydrate " + validators.length + " validators");
