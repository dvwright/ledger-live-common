...
export async function solanaBuildTransaction({
  account,
  core,
  coreAccount,
  coreCurrency,
  ...
  if (isPartial && accountCanEstimateGas) {
    const gasRequest = await core.SolanaGasLimitRequest.init(
      memoTransaction,
      messages,
    ...
    estimatedGas = await libcoreBigIntToBigNumber(
      // NOTE: With new solana code, this call might fail if the account hasn't been synchronized
      ...
      await solanaLikeAccount.estimateGas(gasRequest)
    );
  } else {
  ...
  if (transaction.useAllAmount && transaction.amount) {
    messages = await solanaCreateMessage(
      account.freshAddress,
      {
        ...transaction,
  ...
  const accNum = await solanaLikeAccount.getAccountNumber();
...
export default solanaBuildTransaction;
