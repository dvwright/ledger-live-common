...
import type { Transaction } from "../../families/solana/types";
import { getCurrentSolanaPreloadData } from "../../families/solana/preloadedData";
  ...
  mutations: [
...

    {
      ...
      transaction: ({ account, bridge }) => {
        ...
        invariant(solanaResources, "solana");
        invariant(
          solanaResources.delegations.length < 10,
        ...
        const all = data.validators.filter(
          (v) =>
            !solanaResources.delegations.some(
      ...
      test: ({ account, transaction }) => {
        ...
        transaction.validators.forEach((v) => {
          const d = solanaResources.delegations.find(
...

    {
      ...
      transaction: ({ account, bridge }) => {
        ...
        invariant(solanaResources, "solana");
        invariant(
          solanaResources.delegations.length > 0,
          "already enough delegations"
        );
        const undelegateCandidate = sample(
          solanaResources.delegations.filter(
            (d) =>
              ...
              !solanaResources.unbondings.some(
      ...
      test: ({ account, transaction }) => {
        ...
        invariant(solanaResources, "solana");
        transaction.validators.forEach((v) => {
          const d = solanaResources.unbondings.find(
            (d) => d.validatorAddress === v.address
          );
...

    {
      ...
      transaction: ({ account, bridge }) => {
        ...
        const sourceDelegation = sample(
          solanaResources.delegations.filter((d) => canRedelegate(account, d))
        );
          ...
          updates: [
            {
              mode: "redelegate",
              memo: "LedgerLiveBot",
              solanaSourceValidator: sourceDelegation.validatorAddress,
              validators: [
                {
                  address: delegation.validatorAddress,
      ...
      test: ({ account, transaction }) => {
        ...
        invariant(solanaResources, "solana");
        transaction.validators.forEach((v) => {
          const d = solanaResources.redelegations
            .slice(0)
            // recent first
            ...
            .find(
              (d) =>
                ...
                d.validatorSrcAddress === transaction.solanaSourceValidator
            );
              ...
              validators: [
                {
                  ...
                  // src/families/solana/libcore-buildOperation in the REWARD case
                  amount: BigNumber(0),
                },
              ],
            },
